   ...
   //     print_r($_POST);die;
        switch ( $this->action )
        {
            case 'logout': // /Api/Authentication/Logout
                ...
                unset($myUserAuth);
                break;
            case 'login': // /Api/Authentication/Login
                $myUserAuth = new AuthUser( $this->conn);
            ...
            case 'fetch':
                if ( !empty($this->fetch) )
                {
                    switch ( $this->fetch )
                    {
                            /*
                            */
                        case 'session':  // /Api/Authentication/Session
                            $myUserAuth = new AuthUser( $this->conn);
                            print_r(json_encode($myUserAuth->s));
                            exit;
                        case 'ppw': // /Api/Worksheet
                            $this->ppw();
    ...
    function insertRow($row_title, $row = '', $rowClass = '') {
        $rowData = array();
        $this->altNCh();
        $class = !empty($rowClass) ?  " class=\"$rowClass\"" : '';
        echo "<tr$class>";
        echo "<th class=\"rowName\">$row_title</td>";
        if ( $row != "" )
        {
            if ( is_array($this->worksheetData['stylists']) && !empty($this->worksheetData['stylists']) )
            foreach ( $this->worksheetData['stylists'] as $column )
            {
                echo "<td class=\"stylistColumn\">".$column[$row]."</td>";
            }
            if ( $this->showPeriodSalonAverage() )
            {
                echo "<td>".$this->worksheetData['salon_period_average'][$row]."</td>";
                if ( $this->showStylistAverage() )
                    echo "<td>".$this->worksheetData['stylist_average'][$row]."</td>";
                if ( $this->stylist_id == '' ) echo "<td>".$this->worksheetData['salon_period_total'][$row]."</td>";
                echo "<td>".$this->worksheetData['salon_gross_average'][$row]."</td>";
            }
            if ( !$this->showPeriodSalonAverage() && $this->stylist_id == 0 )
                echo "<td>".$this->worksheetData['salon_period_total'][$row]."</td>";
            
        }
        echo "</tr>";
    }
    function stylistAverage($row_name, $stylist_id = '')
    {
        if ( $row_name != "" && count($this->worksheetData['stylists']) == 1)
        {
            if ( $stylist_id == '' )
            {
                $stylist_id = $this->worksheetData['stylists'][0]['stylist_id'];
            }
            $x = 0;
            $sql = "SELECT `$row_name` FROM pay_period
            ...
            $x = round($x,2);
            $this->worksheetData['stylist_average'][$row_name] = $x;
            return $x;
        }
        return false;
    }
    
    function dropMenu($label = '', $id = '', $onChange = '', $table = '', $sql_id = '', $option_value = '')
    {
        if ( func_num_args() > 0 )
        {
            if ( $label == '' ) $label = '&nbsp;';
            echo "<select name=\"$id\" id=\"$id\" onchange=\"$onChange\">";
            ...
            if ( !empty($table) )
            {
                ...
                while ( $row = $result->fetch_assoc() )
                {
                    echo "<option value=\"".$row[$sql_id]."\"".( !empty(eval('return $this->'.$sql_id.';')) && eval('return $this->'.$sql_id.';') == $row[$sql_id] ? " selected=\"selected\"" : "").">".$row[$option_value]."</option>";
                }
            }
            ?></select><?php
        } else {
    ...
    function ppw()
    {
        ...
        if ( $this->showPeriodSalonAverage() )
        {
            echo "<th>Period Average</th>";
            if ( $this->showStylistAverage() )
            {
                echo "<th>Stylist Average</th>";
            }
            echo "<th>Salon Average</th>";
            if ( $this->stylist_id == '' ) echo "<th>Period Total</th>";
        }
        if ( !$this->showPeriodSalonAverage() && $this->stylist_id == 0 && count($this->worksheetData['stylists']) != 0 )
        {
        ...
        $this->insertRow("Stylist Gross Revenue",'gross_revenue', 'stylistColumn');
        $this->insertRow("Total Product Hours",'pp_product_hours', 'stylistColumn');
        $this->insertRow("Hourly Base Pay",'pp_hourly_base');
        $this->insertRow("Gross Pay based on Hourly Pay",'hourly_gross', 'stylistColumn');
        $this->insertRow("Commissionable Service Total",'pp_commissionable_service', 'stylistColumn');
        $this->insertRow("Service Commission Rate",'pp_service_commission_rate');
        $this->insertRow("Service Commission Earned",'service_commission', 'stylistColumn');
        $this->insertRow("Product Sales",'pp_product_sales', 'stylistColumn');
        ...
        $this->insertRow("Product Commission",'product_commission', 'stylistColumn');
        $this->insertRow("Total Earned Service ",'total_earned_service', 'stylistColumn');
        $this->insertRow("Non Productive Hours ",'pp_nonproduct_hours');
        $this->insertRow("Non Productive Wages ",'nonproduct_wages');
        $this->insertRow("Actual Paycheck",'actual_paycheck', 'stylistColumn');
        $this->insertRow("Divide by Numbers of Production Hours", 'divide_product_hours');
        $this->insertRow("Total Hourly Pay Earned",'total_hourly_pay', 'stylistColumn');
        /*
